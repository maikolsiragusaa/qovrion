...
`src/codex-cache.ts` writes `~/.cache/metrora/codex-results.json` (or `$METRORA_CACHE_DIR/codex-results.json`). Each entry is keyed by absolute file path and validated against `mtimeMs + sizeBytes`. Cached entries are returned wholesale.
