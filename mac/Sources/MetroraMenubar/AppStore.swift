...
private let menubarPeriodDefaultsKey = "MetroraMenubarPeriod"
...
final class AppStore {
    ...
    /// Which Settings tab to show; lets the menu's "About Metrora" item jump
    /// straight to the About tab even when the Settings window is reused.
    var settingsTab: String = "general"
    var displayMetric: DisplayMetric = DisplayMetric(rawValue: UserDefaults.standard.string(forKey: "MetroraDisplayMetric") ?? "") ?? .cost {
        didSet { UserDefaults.standard.set(displayMetric.rawValue, forKey: "MetroraDisplayMetric") }
    }
    var dailyBudget: Double = UserDefaults.standard.double(forKey: "MetroraDailyBudget") {
        didSet { UserDefaults.standard.set(dailyBudget, forKey: "MetroraDailyBudget") }
    }
    ...
    var dailyTokenBudget: Double = UserDefaults.standard.double(forKey: "MetroraDailyTokenBudget") {
        didSet { UserDefaults.standard.set(dailyTokenBudget, forKey: "MetroraDailyTokenBudget") }
    }
    ...
    func clearStaleLoadingIfNeeded() -> Bool {
        ...
        for (key, started) in staleLoading {
            NSLog("Metrora: loading stuck for %ds on %@/%@ — auto-clearing",
                  Int(now.timeIntervalSince(started)), key.label, key.provider.rawValue)
            loadingCountsByKey[key] = nil
            loadingStartedAtByKey[key] = nil
            inFlightKeys[key] = nil
            if cache[key] == nil {
                lastErrorByKey[key] = "Refresh took longer than expected. Metrora will keep retrying in the background."
            }
        }
        for (key, insertedAt) in staleInFlight {
            NSLog("Metrora: orphaned in-flight key stuck for %ds on %@/%@ — clearing",
    ...
    private func invalidateStaleDayCache() {
        ...
        if cacheDate != today {
            ...
            lastErrorByKey.removeAll()
            cacheDate = today
            NSLog("Metrora: reset menubar payload cache for new day %@", today)
        }
    }
    ...
    func setRecoveryExhausted(for label: String) {
        lastErrorByKey[currentKey] = "Could not load \(label). Check that the metrora CLI is installed and working."
    }
    ...
    private func refresh(
        ...
        if staleSeconds.isFinite, staleSeconds > 120 {
            NSLog("Metrora: refresh attempt for stale key \(key.label)/\(key.provider.rawValue) — last success was \(Int(staleSeconds))s ago")
        }
        defer {
        ...
        do {
            ...
            if generationAtStart != payloadRefreshGeneration {
                NSLog("Metrora: dropping fetch result for \(key.label)/\(key.provider.rawValue) — refresh pipeline reset mid-fetch")
                return false
            }
            if Task.isCancelled {
                ...
                NSLog("Metrora: fetch for \(key.label)/\(key.provider.rawValue) cancelled before result was applied")
                return false
            }
            ...
            if cacheDate != cacheDateAtStart || cacheDate != currentCacheDate() {
                invalidateStaleDayCache()
                NSLog("Metrora: dropping fetch result for \(key.label)/\(key.provider.rawValue) — calendar rolled mid-fetch")
                return false
            }
            ...
            if Task.isCancelled { return false }
            NSLog("Metrora: fetch failed for \(key.label)/\(key.provider.rawValue): \(error)")
            if includeOptimize, cache[key] == nil {
                do {
                    ...
                    if Task.isCancelled { return false }
                    NSLog("Metrora: fallback fetch also failed: \(error)")
                }
            }
    ...
    private func refreshQuietly(
        ...
        if key.day == nil && key.period == .today, let age = todayPayloadAgeSeconds, age > 120 {
            NSLog("Metrora: refreshing stale today status payload after %ds", age)
        }
        defer {
            inFlightKeys[key] = nil
        }
        do {
            ...
            if generationAtStart != payloadRefreshGeneration {
                NSLog("Metrora: dropping quiet fetch result for \(key.label) — refresh pipeline reset mid-fetch")
                return false
            }
            ...
            NSLog("Metrora: quiet refresh failed for \(key.label): \(error)")
    ...
    func disconnectSubscription() {
        ...
        NotificationCenter.default.post(name: .metroraSubscriptionDisconnected, object: nil)
    }
...
extension Notification.Name {
    static let metroraSubscriptionDisconnected = Notification.Name("com.metrora.subscriptionDisconnected")
}
