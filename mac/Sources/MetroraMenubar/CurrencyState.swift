...
actor FXRateCache {
    ...
    private var cacheFilePath: String {
        ...
        return base
            .appendingPathComponent("metrora-mac", isDirectory: true)
    ...
    func rate(for code: String) async -> Double? {
        ...
        do {
            ...
            guard let fresh = decoded.rates[code],
                ...
                NSLog("Metrora: discarding out-of-band FX rate for \(code)")
                return entries[code]?.rate
            }
...
/// Reads and writes the CLI's persisted currency config (~/.config/metrora/config.json).
/// Uses an on-disk flock so a concurrent `metrora currency ...` invocation from a terminal
...
enum CLICurrencyConfig {
    private static var configDir: String {
        (NSHomeDirectory() as NSString).appendingPathComponent(".config/metrora")
    }
    ...
    static func persist(code: String) {
        do {
            ...
            NSLog("Metrora: failed to persist currency config: \(error)")
        }
    }
}

struct MetroraCLIConfigStore {
    ...
    private var configDir: String {
        (homeDirectory as NSString).appendingPathComponent(".config/metrora")
    }
    ...
    func persistDevinAcuUsdRate(_ rate: Double) {
        ...
        do {
            ...
            NSLog("Metrora: failed to persist Devin ACU config: \(error)")
        }
    }
}

enum CLIDevinConfig {
    private static let store = MetroraCLIConfigStore()
