...
/// Runs the CLI via argv (no shell interpretation). See `MetroraCLI` for why we never route
...
struct DataClient {
    ...
    private static func runCLI(
        ...
        let process = MetroraCLI.makeProcess(subcommand: subcommand, qualityOfService: qualityOfService)
    ...
    static func runProcess(_ process: Process,
        ...
        timeoutTimer.setEventHandler {
            if process.isRunning {
                NSLog("Metrora: CLI subprocess timed out after %llus for %@ — terminating",
    ...
    private static func drain(_ handle: FileHandle, limit: Int) async -> Data {
        ...
        if flags >= 0 {
            ...
            NSLog("Metrora: fcntl F_GETFL failed on fd %d, drain may block", fd)
        }
        ...
        while buffer.count < limit && !Task.isCancelled {
            ...
            if n > 0 {
                ...
                NSLog("Metrora: drain read() failed on fd %d: errno %d", fd, errno)
                break
            }
        }
        return buffer
