...
private func snapshotsCacheDir() -> String {
    return ProcessInfo.processInfo.environment["METRORA_CACHE_DIR"]
        ?? (NSHomeDirectory() as NSString).appendingPathComponent(".cache/metrora")
}
    ...
    static func record(_ snapshot: SubscriptionSnapshot) async {
        await SnapshotLock.shared.run {
            do {
                ...
                NSLog("Metrora: snapshot record failed: \(error)")
            }
        }
    }
