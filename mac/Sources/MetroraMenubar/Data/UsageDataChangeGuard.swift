...
enum UsageDataChangeGuard {
    ...
    static func snapshot(
        ...
        add(environment["METRORA_DESKTOP_SESSIONS_DIR"] ?? path(homeDirectory, "Library", "Application Support", "Claude", "local-agent-mode-sessions"))
        ...
        let muxHome = expand(environment["METRORA_MUX_DIR"] ?? environment["MUX_ROOT"] ?? path(homeDirectory, ".mux"), homeDirectory: homeDirectory)
        ...
        add(path(homeDirectory, ".config", "metrora", "config.json"), scanFirstLevelDirectories: false)
