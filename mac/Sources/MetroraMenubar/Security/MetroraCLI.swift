...
/// Single entry point for spawning the `metrora` CLI. All callers route through here so the
...
enum MetroraCLI {
    ...
    /// newlines) so a malicious `METRORA_BIN="metrora; rm -rf ~"` can't slip through.
    ...
    private static let userNodePaths: [String] = {
        ...
        if let entries = try? FileManager.default.contentsOfDirectory(atPath: versionsDir) {
            for entry in entries.sorted().reversed() {
                ...
                if FileManager.default.isExecutableFile(atPath: "\(bin)/metrora") {
                    paths.append(bin)
                    break
                }
            }
    ...
    private static let persistedPathFilename = "metrora-cli-path.v1"

    /// Returns the argv that launches the CLI. Dev override via `METRORA_BIN` is honoured only
    /// if every whitespace-delimited token passes `safeArgPattern`. Otherwise falls back to the
    /// plain `metrora` name (resolved via PATH).
    static func baseArgv() -> [String] {
        if ProcessInfo.processInfo.environment["METRORA_ALLOW_DEV_BIN"] == "1",
           let raw = ProcessInfo.processInfo.environment["METRORA_BIN"],
           !raw.isEmpty
        {
            let parts = raw.split(separator: " ", omittingEmptySubsequences: true).map(String.init)
            guard parts.allSatisfy(isSafe) else {
                NSLog("Metrora: refusing unsafe METRORA_BIN; using installed metrora")
    ...
    private static func installedArgv() -> [String] {
        ...
        for candidate in (additionalPathEntries + userNodePaths).map({ "\($0)/metrora" }) {
    ...
    private static func persistedCLIPath() -> String? {
        ...
        let url = support
            .appendingPathComponent("Metrora", isDirectory: true)
    ...
    static func makeProcess(
        ...
        // metrora subprocess parses 5-10x slower than the same command run from a
