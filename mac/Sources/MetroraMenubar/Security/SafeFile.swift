...
///
/// Every cache file we touch (`~/Library/Caches/metrora-mac/fx-rates.json`,
/// `~/.cache/metrora/subscription-snapshots.json`, `~/.config/metrora/config.json`) is a
...
enum SafeFile {
    ...
    static func write(_ data: Data, to path: String, mode: mode_t = 0o600) throws {
        ...
        let tmpPath = parent + "/.metrora-" + UUID().uuidString + ".tmp"
    ...
    /// metrora processes (the CLI running in a terminal, say) block on the same file instead
