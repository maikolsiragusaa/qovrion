...
/// must be whitespace-joined argv where each token passes `MetroraCLI.isSafe` (regex allowlist
...
enum TerminalLauncher {
    ...
    static func open(subcommand: [String]) {
        let argv = MetroraCLI.baseArgv() + subcommand
        guard argv.allSatisfy(MetroraCLI.isSafe) else {
            NSLog("Metrora: refusing to open terminal with unsafe argv")
            return
        }
        ...
        let headless = MetroraCLI.makeProcess(subcommand: subcommand)
        try? headless.run()
    }

    /// Launches `claude login` in Terminal.app so the user can complete the OAuth flow
    /// without leaving Metrora. The command is a hardcoded literal -- no user input is
    ...
    static func openClaudeLogin() -> Bool {
        guard terminalPaths.contains(where: FileManager.default.fileExists(atPath:)) else {
            NSLog("Metrora: Terminal.app not present; user must run `claude login` manually")
            return false
        }
    ...
    private static func runInTerminal(command: String, preValidated: Bool) {
        if !preValidated {
            ...
            guard tokens.allSatisfy(MetroraCLI.isSafe) else {
                NSLog("Metrora: refusing to run unvalidated command in Terminal")
                return
            }
        }
        let script = """
