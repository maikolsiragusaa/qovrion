...
private let starBannerGitHubURL = URL(string: "https://github.com/maikolsiragusaa/metrora")!
...
struct StarBanner: View {
    @AppStorage("metrora.starBannerDismissed") private var dismissed: Bool = false
...
struct FooterBar: View {
    ...
    /// Runs `metrora export` directly into ~/Downloads and reveals the result in Finder. CSV
    ...
    private func runExport(format: ExportFormat) {
        Task {
            ...
            let base = "metrora-\(formatter.string(from: Date()))"
            let outputPath = (downloads as NSString).appendingPathComponent(base + format.suffix)

            let process = MetroraCLI.makeProcess(subcommand: [
                ...
                process.terminationHandler = { proc in
                    Task { @MainActor in
                        if proc.terminationStatus == 0 {
                            ...
                            NSLog("Metrora: \(fmt.cliName.uppercased()) export exited with status \(proc.terminationStatus)")
                        }
                    }
                }
                try process.run()
            } catch {
                NSLog("Metrora: \(format.cliName.uppercased()) export failed: \(error)")
            }
        }
    }
    ...
    /// Instant-feeling currency switch. Updates the symbol and any cached FX rate on the main
     ...
     /// background. CLI config is persisted so other metrora commands stay in sync.
