...
struct AppStoreRefreshRecoveryTests {
    ...
    func dailyBudgetWarningIsSuppressedForCombinedScope() {
        ...
        let previousDisplayMetric = defaults.object(forKey: "MetroraDisplayMetric")
        let previousDailyBudget = defaults.object(forKey: "MetroraDailyBudget")
        defer {
            if let previousDisplayMetric {
                defaults.set(previousDisplayMetric, forKey: "MetroraDisplayMetric")
            } else {
                defaults.removeObject(forKey: "MetroraDisplayMetric")
            }
            if let previousDailyBudget {
                defaults.set(previousDailyBudget, forKey: "MetroraDailyBudget")
            } else {
                defaults.removeObject(forKey: "MetroraDailyBudget")
            }
        }
